 ...
 * @LastEditTime: 2023-06-15 20:49:58
 * @FilePath: \frontend\src\views\admin\AdmEmp.vue
 * @Description:  
 * 
-->
<template>
  <div class="table-wrapper">
    ...
    >
      <el-form model="form" ref="form">
        <el-form-item label="员工号" prop="id" label-width="100px">
        ...
        <el-form-item label="部门" prop="position" label-width="100px">
          <el-select v-model="form_position" placeholder="department">
            <el-option
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleAdd = () => {
      ...
      const max = empData.value.reduce((acc, item) => {
    ...
    const displayData = computed(() => {
      ...
      return empData.value.slice(start, end);
    });
    ...
    const handleDelete = (index: number, row: Employee) => {
      ElMessageBox.confirm("数据将会永久删除(真的很久！)", "警告", {
        ...
        .then(() => {
          axios
            .delete(`http://localhost:7000/admin/employee/delete${row.id}`)
            .then((response) => {
    ...
    const editConfirm = () => {
      ...
      if (form_pid.value.length < 20 && form_phone.value.length < 20) {
        axios
          .post(
            "http://localhost:7000/admin/employee/edit",
            {
              postData,
            },
            {
    ...
    const addConfirm = () => {
      ...
      axios
        .post(
          "http://localhost:7000/admin/employee/add",
          {
            postData,
          },
          {
